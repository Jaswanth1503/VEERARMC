    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black text-charcoal-black flex items-center gap-2">
          <FileText className="w-6 h-6 text-accent-orange" /> Invoices & Billing Statements
        </h1>
        <p className="text-xs text-concrete-600 mt-1">
          View GST tax invoices, payment histories, and download PDF receipts for Ready Mix Concrete supplies.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-concrete-200 p-12 text-center space-y-4">
          ...
          <div className="text-sm font-bold text-charcoal-black">No invoices issued yet</div>
          <p className="text-xs text-concrete-500 max-w-sm mx-auto">
            Invoices are automatically generated upon batching dispatch confirmation.
          </p>
        </div>
      ) : (
                  ...
                  <tr key={inv.id} className="hover:bg-concrete-50/50 transition-colors">
                    ...
                    <td className="p-4">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-extrabold ${
                        inv.status === "PAID" ? "bg-emerald-500/15 text-emerald-700" : "bg-amber-500/15 text-amber-700"
                      }`}>
                        {inv.status}
                      </span>
                    </td>
