...
export const ChatWidget: React.FC = () => {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
          ...
          if (dataJson) {
            try {
              ...
              if (eventType === "metadata") {
                ...
                streamedText = `⚠️ ${data.message || "An error occurred."}`;
              }
            } catch (e) {
      ...
      setMessages(prev =>
        prev.map(m =>
          m.id === assistantMsgId
            ? { ...m, content: "⚠️ Veera AI is temporarily unavailable. Please try again shortly.", isStreaming: false }
            : m
        )
      );
    } finally {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {isOpen && (
        <div className="mb-4 w-[380px] sm:w-[420px] h-[520px] rounded-2xl bg-white border border-concrete-200 shadow-2xl shadow-charcoal-black/20 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-200">
          {/* Header */}
          <div className="p-3.5 bg-concrete-50 border-b border-concrete-200 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-concrete-500">RAG Grounded Intelligence</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1">
              <Link
                ...
                className="p-1.5 rounded-lg text-concrete-500 hover:text-accent-orange hover:bg-concrete-100 transition-colors"
              ...
              <button
                onClick={() => setIsOpen(false)}
                className="p-1.5 rounded-lg text-concrete-500 hover:text-charcoal-black hover:bg-concrete-100 transition-colors"
                title="Close"
              >
                <X className="w-4 h-4" />
              </button>
                ...
                <div className="pt-2 flex flex-col gap-1.5 text-left">
                  {[
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-white border border-concrete-200 hover:border-accent-orange/50 text-xs text-concrete-800 hover:text-accent-orange transition-all text-left flex items-center justify-between shadow-sm"
                    >
                      <span>{q}</span>
                ...
                <div
                  className={`p-3 rounded-2xl max-w-[85%] ${
                    m.role === "user"
                      ...
                      : "bg-white border border-concrete-200 text-concrete-800 shadow-sm"
                  }`}
                >
          ...
          <form
            ...
            className="p-2 bg-white border-t border-concrete-200 flex items-center gap-2"
          >
            <input
              type="text"
              value={input}
            ...
            <button
              ...
              className="p-2 rounded-xl bg-accent-orange text-white hover:bg-accent-orange/90 disabled:opacity-40 transition-all shadow-md shrink-0"
            >
      ...
      <button
        ...
        className="group relative p-3.5 rounded-2xl bg-accent-orange text-white shadow-xl shadow-orange-950/20 hover:scale-105 active:scale-95 transition-all duration-200 flex items-center justify-center"
